...
| ECDSA key for Txn | `/generateEcDSAKeys` | ECDSA key for txn of token from one node to next | ```curl --header "Content-Type: application/json" --request POST 'http://localhost:1898/generateEcDSAKeys' --data '{ "pvtKeyPass" : "<password>" , "returnKey" : 0}'``` |
| ECDSA key for Quorum | `/generateQuorumKeys` | ECDSA key for node to act as quorum | ```curl --header "Content-Type: application/json" --request POST 'http://localhost:1898/generateQuorumKeys' --data '{ "pvtKeyPass" : "<password>" , "returnKey" : 0}'``` |
